...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!scheduledRoomId) {
      return jsonError(400, 'scheduledRoomId is required');
    }
    ...
    if (existingRows?.length) {
      return NextResponse.json({ ok: true, alreadySubmitted: true, session: existingRows[0] });
    }
    ...
    if (roomError || !room) {
      return jsonError(404, roomError?.message ?? 'Room not found');
    }
    ...
    if (insertError) return jsonError(500, insertError.message);
    ...
    if (userId) {
      const { data: profile } = await supabaseAdmin.from('profiles').select('xp_total').eq('user_id', userId).maybeSingle();
      if (profile) {
      ...
      return NextResponse.json({ ok: true, session: inserted, xpAwarded, streak: nextCurrent, attendanceRatio });
    }
